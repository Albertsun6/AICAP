...
```bash
# （可选）创建项目级日志目录
# For each stale plan: mv .skill-data/ceo-plans/{old-plan}.md .skill-data/ceo-plans/archive/
```

Write to `.skill-data/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```
NOTE: These represent human-team implementation hours. With AI Agent,
...
### TODOS.md updates
Present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `references/TODOS-format.md`.
...
* **Effort estimate:** S/M/L/XL (human team) → with AI Agent: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
- **B)** Keep in `~/.skill-data/projects/` only (local, personal reference)
- **C)** Skip

...
> 版本：rulesync@16.2.0 ｜ 文档日期：2026-07-30（版本随升级更新）
...
flowchart TD
    ...
    GEN["rulesync generate 适配生成层 pin 16.2.0"]
...
```bash
cd AICAP
pnpm install                 # 装 pin 住的 rulesync@16.2.0（package.json 已声明）
...
| `npx rulesync@16.2.0 import --targets claudecode` | 把已有的工具配置反向导入 `.rulesync/`（迁移老项目用） |
...
| **版本 pin** | `package.json` + CI 均 `rulesync@16.2.0` | 不同 rulesync 版本生成结果可能不同，pin 住避免假阳性漂移；升级走 PR 并复核 diff |
...
A：说明你改了源但没重新生成，或本地 rulesync 版本与 pin 的不一致。先 `pnpm install` 确认版本=16.2.0，再 `pnpm run ai:generate`，提交。
...
### 12.3 已验证状态
`rulesync@16.2.0 init → 填充 SSOT → generate → ai:check` 端到端跑通：Claude Code(`CLAUDE.md`)、Codex/Cursor(`AGENTS.md`)、Cursor(`.cursor/rules/*.mdc`)、`.mcp.json` 均正确生成；重生成幂等（无漂移），CI drift gate 逻辑成立。源文件均为可直接替换的真实模板。

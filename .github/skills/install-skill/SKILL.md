...
name: install-skill
description: >-
  从 anthropics/skills 等主流 skill market 安装新技能到 AICAP SSOT： fetch → 展示新增列表 →
  用户选择保留项 → 删除其余 → generate → symlink，一步到位。 触发：安装 skill / 从 anthropics 安装 /
  install skill from market / 拉取新 skill / 我想装个官方 skill
---
# install-skill

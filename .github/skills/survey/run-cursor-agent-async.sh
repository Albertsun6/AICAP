#!/usr/bin/env bash
# /survey — cursor-agent 长任务异步 job 机制（start / status / wait 三段式）
#
# 为什么要有这个文件：Claude Code Bash 工具单次调用硬上限 600s 且不可调
# （BASH_MAX_TIMEOUT_MS 有已知 bug 不生效，issue #34138/#25881 均 not planned），
# run-cursor-agent.sh 的同步 570s watchdog 因此被钉死。但 Phase 6 全文评审这类
# 重活在 xhigh 档常要 8-15 分钟——同步窗口装不下，超时=白烧配额+整段丢失（实测踩过）。
# 解法：nohup 把任务脱离 Claude Code 进程树，deadline 想设多长设多长；
# 主 Claude 用**有界 wait 波次**（每波 <570s，可反复续杯）拿完成通知，期间照常干别的活。
#
# 用法：
#   bash run-cursor-agent-async.sh start <prompt-file> <output-file> [family] [deadline-sec]
#       → stdout 打 JOB_DIR 路径后立即返回。deadline 默认 1800s（30 min），上限 7200
#   bash run-cursor-agent-async.sh status <job-dir>
#       → 一行状态：RUNNING <已跑>s/<deadline>s | DONE | FAILED exit=<N>（附 stderr 尾部）| DIED
#       exit code：0=DONE  3=RUNNING  4=FAILED/DIED
#   bash run-cursor-agent-async.sh wait <job-dir> [max-wait-sec]
...
#   4. DONE → Read <output-file>；FAILED/DIED → 按 run-cursor-agent.sh 的 exit code
#      映射降级（124=超时 / 65=调用失败 / 66=空输出），fail-loud，不许"还在跑"含混
#
...
case "$CMD" in start|status|wait) ;; *)
  echo "Usage: $0 start <prompt-file> <output-file> [gpt|gemini|grok] [deadline-sec]" >&2
...
# ── start ────────────────────────────────────────────────────────────────
if [ "$CMD" = start ]; then
  if [ $# -lt 2 ] || [ $# -gt 4 ]; then echo "ERROR: start 需要 2-4 个参数" >&2; exit 64; fi
  PROMPT_FILE="$1"; OUTPUT_FILE="$2"; FAMILY="${3:-gpt}"; DEADLINE="${4:-1800}"
...
print_status() { # → exit 0=DONE 3=RUNNING 4=FAILED/DIED
  ...
  if [ -f "$JOB_DIR/job.exit" ]; then
    ...
    if [ "$code" = "0" ] && [ -s "$OUTPUT_FILE" ]; then
      echo "DONE  耗时 ${elapsed}s  输出 $(wc -c < "$OUTPUT_FILE" | tr -d ' ') bytes → $OUTPUT_FILE"
      return 0
    fi
    echo "FAILED exit=$code 耗时 ${elapsed}s（124=超deadline / 65=调用失败 / 66=空输出）—— job.log 尾部："
